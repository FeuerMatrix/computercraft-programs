...
local ExcavationTree = require "ExcavationTree"
...
function check()
    ...
    if excavation_graph == nil then
        excavation_graph = ExcavationTree.newInstance(x, y, z)
    end
    local temp = ExcavationTree.newInstance(x + xdir, y, z + zdir)
...
function checkUp()
    ...
    local temp = ExcavationTree.newInstance(x, y+1, z)
    excavation_graph:addChild(temp)
    excavation_graph = temp
    up()
    cascade_checks()
